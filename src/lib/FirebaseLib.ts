...
export class FirebaseLib {
  ...
  /**
   * Creates or updates a Firestore document by merging data with the existing document.
   * 
   * @param path The collection path
   * @param docID The document ID
   * @param data The data to set/merge
   * @returns A WriteResult object or null if failed
   */
  async setData(path: string, docID: string, data: Record<string, any>): Promise<FirebaseFirestore.WriteResult | null> {
    try {
      const docRef = this.firestore.collection(path).doc(docID);
      const result = await docRef.set(data, { merge: true });
      return result;
    } catch (error) {
      console.error(`Error setting Firestore data at ${path}/${docID}:`, error);
      return null;
    }
  }
}