    ...
    try {
        do {
            ...
            if (!isFullArray(result)) {
                console.log(`No more documents to process in ${collection}`);
                break;
            }
            ...
            if (!lastTimestamp || (offset !== null && lastTimestamp === offset)) {
                console.log(`Reached end of pagination or duplicate timestamp in ${collection}`);
                break;
            }
            ...
            console.log(`Writing ${processedResults.length} documents to Meilisearch for ${collection}`);
        ...
        console.log(`Transaction hydration completed successfully for ${collection}. Total batches: ${output.length}`);
        return output;
    } catch (error) {
        console.error(`Error during transaction hydration for ${collection}:`, error);
        throw error;
    }
}